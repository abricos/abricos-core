        ...
        def parseDir(path, tree):
            ...
            for i in os.listdir(path):
                ...
                if os.path.isdir(fullname):
                    subdirsname.append(i)
                    # subdirs.append(fullname)
                elif i.lower().endswith(self.extension):
                    dircontent += parseFile(path, i, tree)

            for i in subdirsname:
                fullname = os.path.join(path, i)
                log.info("           "+i);
                dircontent += parseDir(fullname, tree+"--"+i)
...
it was empty" % token
        ...
        if const.FILE_MARKER in tokenMap:
            ...
            file_marker_arr = tokenMap[const.FILE_MARKER][0].split(" ")
            file_name = file_marker_arr[0]
            self.currentFile = file_name
            target = { const.NAME: file_name, const.CLASS_LIST:[], const.FILE_NAME: file_marker_arr[1] }
