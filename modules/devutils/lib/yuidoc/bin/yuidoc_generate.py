...
class DocGenerator(object):
    ...
    def __init__(self, inpath, datafile, outpath, templatepath, newext, showprivate=False, 
                 projectname='Abricos! User Interface Library', 
                 version='', 
                 projecturl='http://abricos.org', 
                 ydn=False):
        ...
        for i in os.listdir(self.templatepath):
            ...
            if os.path.isdir(fullname) and i != ".svn":
                targetdir = os.path.join(self.outpath, i)
                try:
                    shutil.rmtree(targetdir)
                except: pass
                _copytree(fullname, targetdir)
    ...
    def process(self):
        ...
        def assignGlobalProperties(template):
            ...
            template.viewfilename = self.viewfilename
            if self.filename:
            ...
            mymy = []
            mymy.append({"k": "one", "v": "value one"})
            mymy.append({"k": "two", "v": "value two"})
            mymy.append({"k": "tree", "v": "value tree"})
            
            '''
            mymy['one'] = 'value one'
            mymy['two'] = 'value two'
            mymy['tree'] = 'value tree'
            '''
        ...
        for mname in self.modules:
            ...
            filenames = m[const.FILE_LIST]
            filenames.sort(soft_sort)
        
            self.filenames = [];
            for i in filenames:
                self.filenames.append({const.FILE: i, const.FILE_NAME: self.data[const.FILE_MAP][i][const.FILE_NAME]}) 
            ...
            for i in m[const.FILE_LIST]:
                ...
                self.viewfilename = self.data[const.FILE_MAP][i][const.FILE_NAME]
                assignGlobalProperties(t)
        ...
        filenames = self.data[const.FILE_MAP].keys()
        filenames.sort(soft_sort)
