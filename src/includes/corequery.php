...
class Ab_CoreQuery {
    ...
    public static function ModuleUpdateLanguageVersion(Ab_Database $db, Ab_Module $module){
        $findCol = false;
        $lngId = 'language_'.Abricos::$LNG;
        $sql = "SHOW COLUMNS FROM ".$db->prefix."module";
        ...
        if (!$findCol){
            $db->query_write("ALTER TABLE ".$db->prefix."module ADD ".$lngId." varchar(20) NOT NULL default '0.0'");
        }
        $db->query_write("
			UPDATE ".$db->prefix."module
			SET ".$lngId."='".bkstr($module->version)."'
			WHERE name='".bkstr($module->name)."'
		");
    }
