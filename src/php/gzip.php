...
if ($libType == 'sys'){
    ...
    while (false !== ($entry = $dir->read())) {
        ...
        $jsFilesInfo = $realPath."/modules/".$entry."/js/files.json";
        if (!file_exists($jsFilesInfo)){
            continue;
        }
        $content .= "\nv=[];\n";

        $infoContent = file_get_contents($jsFilesInfo);
        $data = json_decode($infoContent);
        if (!isset($data->files)){
            continue;
        }
        foreach ($data->files as $fName => $fVersion){
            $content .= "v[v.length]={f:'".$fName.".js', k:'".$fVersion."'};\n";
        }
        $content .= "m['".$entry."']=v;\n";
    }
    $content .= "
